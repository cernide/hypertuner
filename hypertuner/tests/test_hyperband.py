...
class TestHyperbandSearchManager(BaseTestCase):
    ...
    def test_get_num_runs_to_keep(self):
        ...
        # Iteration == 0
        assert (
            self.manager1.config.get_num_runs_to_keep(
                num_runs=9, bracket_iteration=0)
            == 3
        )
        assert (
            self.manager1.config.get_num_runs_to_keep(
                num_runs=9, bracket_iteration=1)
            == 1
        )
        assert (
            self.manager1.config.get_num_runs_to_keep(
                num_runs=9, bracket_iteration=2)
            == 0
        )
        ...
        assert (
            self.manager1.config.get_num_runs_to_keep(
                num_runs=5, bracket_iteration=0)
            == 1
        )
        assert (
            self.manager1.config.get_num_runs_to_keep(
                num_runs=5, bracket_iteration=1)
            == 0
        )
        ...
        assert (
            self.manager1.config.get_num_runs_to_keep(
                num_runs=3, bracket_iteration=0)
            == 1
        )
        assert (
            self.manager1.config.get_num_runs_to_keep_for_iteration(
        ...
        # Iteration == 0
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=81, bracket_iteration=0)
            == 27
        )
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=81, bracket_iteration=1)
            == 9
        )
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=81, bracket_iteration=2)
            == 3
        )
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=81, bracket_iteration=3)
            == 1
        )
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=81, bracket_iteration=4)
            == 0
        )
        ...
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=34, bracket_iteration=0)
            == 11
        )
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=34, bracket_iteration=1)
            == 3
        )
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=34, bracket_iteration=2)
            == 1
        )
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=34, bracket_iteration=3)
            == 0
        )
        ...
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=15, bracket_iteration=0)
            == 5
        )
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=15, bracket_iteration=1)
            == 1
        )
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=15, bracket_iteration=2)
            == 0
        )
        ...
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=8, bracket_iteration=0)
            == 2
        )
        assert (
            self.manager2.config.get_num_runs_to_keep(
                num_runs=8, bracket_iteration=1)
            == 0
        )
    ...
    def test_get_resources(self):
        ...
        self.almost_equal(
            self.manager1.get_resources_for_iteration(iteration=0), 1.11)
        self.almost_equal(
            self.manager1.get_resources_for_iteration(iteration=1), 3.33)
        self.almost_equal(
            self.manager1.get_resources_for_iteration(iteration=2), 10)
        ...
        self.almost_equal(
            self.manager2.get_resources_for_iteration(iteration=0), 1)
        self.almost_equal(
            self.manager2.get_resources_for_iteration(iteration=1), 3)
        self.almost_equal(
            self.manager2.get_resources_for_iteration(iteration=2), 9)
        self.almost_equal(
            self.manager2.get_resources_for_iteration(iteration=3), 27)
        self.almost_equal(
            self.manager2.get_resources_for_iteration(iteration=4), 81)
    ...
    def test_get_n_resources(self):
        ...
        # Iteration == 0
        self.almost_equal(
            self.manager1.get_n_resources(
                n_resources=1.11, bracket_iteration=0), 1.11
        )
        self.almost_equal(
            self.manager1.get_n_resources(
                n_resources=1.11, bracket_iteration=1), 3.33
        )
        self.almost_equal(
            self.manager1.get_n_resources(
                n_resources=1.11, bracket_iteration=2), 9.99
        )
        ...
        self.almost_equal(
            self.manager1.get_n_resources(
                n_resources=3.33, bracket_iteration=0), 3.33
        )
        self.almost_equal(
            self.manager1.get_n_resources(
                n_resources=3.33, bracket_iteration=1), 9.99
        )
        ...
        self.almost_equal(
            self.manager1.get_n_resources(
                n_resources=9.99, bracket_iteration=0), 9.99
        )
        ...
        # Iteration == 0
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=1, bracket_iteration=0), 1
        )
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=1, bracket_iteration=1), 3
        )
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=1, bracket_iteration=2), 9
        )
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=1, bracket_iteration=3), 27
        )
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=1, bracket_iteration=4), 81
        )
        ...
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=3, bracket_iteration=0), 3
        )
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=3, bracket_iteration=1), 9
        )
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=3, bracket_iteration=2), 27
        )
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=3, bracket_iteration=3), 81
        )
        ...
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=9, bracket_iteration=0), 9
        )
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=9, bracket_iteration=1), 27
        )
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=9, bracket_iteration=2), 81
        )
        ...
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=27, bracket_iteration=0), 27
        )
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=27, bracket_iteration=1), 81
        )
        ...
        self.almost_equal(
            self.manager2.get_n_resources(
                n_resources=81, bracket_iteration=0), 81
        )
    ...
    def test_should_reduce_configs(self):
        assert (
            self.manager1.config.should_reduce_configs(
                iteration=0, bracket_iteration=0)
            is True
        )
        assert (
